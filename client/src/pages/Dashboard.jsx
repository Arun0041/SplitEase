    ...
    <div style={{ animation: 'slide-up 0.4s ease-out' }}>
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-brand-600 to-brand-500 text-white font-bold rounded-xl hover:-translate-y-0.5 shadow-md hover:shadow-lg hover:shadow-brand-500/20 transition-all"
        >
      ...
      {groups.length === 0 ? (
        <div className="text-center py-20 glass rounded-3xl shadow-sm">
          <div className="mx-auto w-16 h-16 bg-brand-50 text-brand-600 rounded-2xl flex items-center justify-center mb-4 text-3xl border border-brand-100">
            🏖️
          </div>
          ...
          <button
            onClick={() => setShowModal(true)}
            className="mt-6 px-5 py-2.5 font-bold text-brand-700 bg-brand-50 border border-brand-100 rounded-xl hover:bg-brand-100 transition-colors"
          >
            Create First Group
          </button>
        </div>
            ...
            >
              ...
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className={`w-12 h-12 bg-gradient-to-br ${colors[i % colors.length]} rounded-xl flex items-center justify-center text-white font-extrabold text-xl shadow-md`}>
                    {g.name.charAt(0)}
                  </div>
                  <span className="text-xs font-bold bg-slate-100 text-slate-500 border border-slate-200 px-3 py-1 rounded-full">
          ...
          >
            ...
            <form onSubmit={handleCreate}>
              <div className="px-7 py-6 space-y-5 bg-slate-50/50">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl font-medium text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-400 transition-all shadow-sm"
              ...
              <div className="px-7 py-5 bg-white border-t border-slate-100 flex gap-3 justify-end">
                <button type="button" onClick={() => setShowModal(false)} className="px-5 py-2.5 font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors">Cancel</button>
                <button type="submit" className="px-5 py-2.5 font-bold text-white bg-gradient-to-r from-brand-600 to-brand-500 rounded-xl shadow-md hover:shadow-lg hover:shadow-brand-500/20 transition-all">Create Group</button>
              </div>
            </form>
          </div>
        </div>
